var logger = require('pofresh-logger').getLogger('pofresh-admin', 'MasterAgent');
var EventEmitter = require('events').EventEmitter;
var MqttServer = require('../protocol/mqtt/mqttServer');
var SioServer = require('../protocol/socketio/sioServer');
...
MasterAgent.prototype.doAuthUser = function (msg, socket, cb) {
    ...
    this.consoleService.authUser(msg, env, function (user) {
        if (!user) {
            // client should auth with username
            socket.send('register', {
                code: protocol.PRO_FAIL,
...
MasterAgent.prototype.doAuthServer = function (msg, socket, cb) {
    var self = this;
    var env = self.consoleService.env;
    this.consoleService.authServer(msg, env, function (status) {
        if (status !== 'ok') {
            socket.send('register', {
                code: protocol.PRO_FAIL,
